...
async function getConversations(status?: string, page = 1) {
  const pageSize = 20;
...
export default async function AdminConversationsPage({ searchParams }: ConversationsPageProps) {
  const params = await searchParams;
  const status = params.status || 'all';
  const page = parseInt(params.page || '1');

  const [{ conversations, total }, statusCounts] = await Promise.all([
    getConversations(status, page),
    getStatusCounts(),
  ]);
    ...
    <div className="p-6">
      ...
      <div className="flex items-center gap-2 mb-6 border-b border-gray-200 pb-4">
        {tabs.map((tab) => (
          <Link
            key={tab.id}
            href={`/admin/conversations?status=${tab.id}`}
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between mt-6">
          ...
          <div className="flex gap-2">
            {page > 1 && (
              <Link href={`/admin/conversations?status=${status}&page=${page - 1}`}>
