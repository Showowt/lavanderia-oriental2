...
async function getOrders(status?: string, page = 1) {
  ...
  let query = supabaseAdmin
    .from('orders')
    .select(`*, customer:customers(id, name, phone), location:locations(id, name)`, { count: 'exact' })
  ...
  return { orders: (data || []) as OrderListItem[], total: count || 0 };
}
...
async function getStatusCounts() {
  ...
  const results = await Promise.all([
    ...
    ...statuses.map((s) =>
      supabaseAdmin.from('orders').select('*', { count: 'exact', head: true }).eq('status', s)
    ),
  ]);
